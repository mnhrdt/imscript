...
static void fill_gblur_gray(float *y, float *x, int w, int h, float s)
{
	gblur_gray(y, x, w, h, s);
}

static void fill_gblur_split(float *y, float *x, int w, int h, int pd, float s)
{
	// assumes "split", not "vec"
	for (int i = 0; i < pd; i++)
		fill_gblur_gray(y + i*w*h, x + i*w*h, w, h, s);
}


// fill a pyramid of images of the same size
void pyrss(float *p, float *x, int w, int h, int pd, int n, float sstep)
{
	assert(sstep > 1);
	...
	for (int i = 0; i < n; i++)
	{
		...
		fill_gblur_split(p + i*w*h*pd, x, w, h, pd, s);
		s *= sstep;
	}
}
