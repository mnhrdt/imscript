...
static void hs_init_iteration(float *u, float *v,
		float *init, float *mask,
		...
		int niter, float alpha)
{
	...
	for (int i = 0; i < niter; i++)
		hs_init_iteration(u, v, init, mask, gx, gy, gt, w, h, alpha);
	free(gx);
	free(gy);
       	free(gt);
}

int hs_init_stopping(float *u, float *v,
		float *init, float *mask,
		float *a, float *b, int w, int h,
...
int main(int argc, char *argv[])
{
	...
	char *filename_m = argv[argc-5];
	char *filename_i = argv[argc-4];
	...
	float *mask = iio_read_image_float_vec(filename_m, &ww, &hh, &mpd);
	if (w != ww || h != hh)
		exit(fprintf(stderr, "input images size mismatch\n"));
	float *init = iio_read_image_float(filename_i, &ww, &hh);
	if (w != ww || h != hh)
		exit(fprintf(stderr, "input images size mismatch\n"));
	...
	if (isfinite(epsilon)) {
		int nit = hs_init_stopping(u, v, initu, initv, mask,
				a, b, w, h, niter, alpha, epsilon);
		fprintf(stderr, "ran %d iterations\n", nit);
	} else
		hs_init(u, v, initu, initv, mask, a, b, w, h, niter, alpha);
