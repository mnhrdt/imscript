		...
		float *a, float *b, int w, int h)
{
	...
	// TODO: probably it is better to extend images by constant, but the
...
// Run a range of iterations with fixed "alpha", saving snapshots
static void hs_snapshots(char *filename_pattern,
		float *a, float *b, int w, int h,
		float alpha)
{
	float *gx = xmalloc(w * h * sizeof(float));
...
// command-line interface for running with fixed alpha, with several niter
static int main_multiple(int argc, char *argv[])
{
	if (argc <= 5) {
		fprintf(stderr, "usage:\n\t%s a b fpattern alpha n1 n2 ...\n",
									*argv);
		//                          0 1 2 3        4     5  6 ...
		return EXIT_FAILURE;
	}
...
// command-line interface for running with fixed (niter,alpha)
static int main_single(int argc, char *argv[])
{
	if (argc != 6) {
		fprintf(stderr, "usage:\n\t%s niter alpha a b f\n", *argv);
		return EXIT_FAILURE;
