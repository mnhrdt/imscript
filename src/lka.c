...
static void compute_affine_structure_tensor_here(
		float atwa[6][6], float arhs[6],
		float *wv, int (*wo)[2], int kside,
		float *gx, float *gy, int w, int h,
		int ii, int jj)
{
	...
	for (int k = 0; k < n; k++)
	{
		double x = ii + wo[k][0];
		double y = jj + wo[k][1];
		double Ex = p(gx, w, h, x, y);
		double Ey = p(gy, w, h, x, y);
		double Et = p(gt, w, h, x, y);
		//int ii = i + wo[k][0];
		//int jj = j + wo[k][1];
		//atwa[0] += wv[k] * sqr(p(gx, w, h, ii, jj));
		//atwa[1] += wv[k] * p(gx,w,h, ii, jj) * p(gy,w,h, ii, jj);
		...
		//	double Ey = p(gy, w, h, i, j);
		//	double Et = p(gt, w, h, i, j);
		ast[0][0] += wv[k] * Ex * Ex * x * x;
		ast[0][1] += wv[k] * Ex * Ex * x * y;
		ast[0][2] += wv[k] * Ex * Ex * x;
		ast[0][3] += wv[k] * Ex * Ey * x * x;
		ast[0][4] += wv[k] * Ex * Ey * x * y;
		ast[0][5] += wv[k] * Ex * Ey * x;
		ast[1][0] += wv[k] * Ex * Ex * x * y;
		ast[1][1] += wv[k] * Ex * Ex * y * y;
		ast[1][2] += wv[k] * Ex * Ex * y;
		ast[1][3] += wv[k] * Ex * Ey * x * y;
		ast[1][4] += wv[k] * Ex * Ey * y * y;
		ast[1][5] += wv[k] * Ex * Ey * y;
		ast[2][0] += wv[k] * Ex * Ex * x;
		ast[2][1] += wv[k] * Ex * Ex * y;
		ast[2][2] += wv[k] * Ex * Ex;
		ast[2][3] += wv[k] * Ex * Ey * x;
		ast[2][4] += wv[k] * Ex * Ey * y;
		ast[2][5] += wv[k] * Ex * Ey;
		ast[3][0] += wv[k] * Ex * Ey * x * x;
		ast[3][1] += wv[k] * Ex * Ey * x * y;
		ast[3][2] += wv[k] * Ex * Ey * x;
		ast[3][3] += wv[k] * Ey * Ey * x * x;
		ast[3][4] += wv[k] * Ey * Ey * x * y;
		ast[3][5] += wv[k] * Ey * Ey * x;
		ast[4][0] += wv[k] * Ex * Ey * x * y;
		ast[4][1] += wv[k] * Ex * Ey * y * y;
		ast[4][2] += wv[k] * Ex * Ey * y;
		ast[4][3] += wv[k] * Ey * Ey * x * y;
		ast[4][4] += wv[k] * Ey * Ey * y * y;
		ast[4][5] += wv[k] * Ey * Ey * y;
		ast[5][0] += wv[k] * Ex * Ey * x;
		ast[5][1] += wv[k] * Ex * Ey * y;
		ast[5][2] += wv[k] * Ex * Ey;
		ast[5][3] += wv[k] * Ey * Ey * x;
		ast[5][4] += wv[k] * Ey * Ey * y;
		ast[5][5] += wv[k] * Ey * Ey;
		rhs[0] -= wv[k] * Et * Ex * x;
		rhs[1] -= wv[k] * Et * Ex * y;
		rhs[2] -= wv[k] * Et * Ex;
...
static void least_squares_ofca(float *u, float *v,
		float *a, float *b, int w, int h,
...
int main(int argc, char *argv[])
{
	...
	least_squares_ofca(u, v, a, b, w, h, kside, sigma);
