...
void gblur_symmetric_gray(float *y, float *x, int w, int h, float s)
{
	float (*xs)[2*w] = xmalloc(4 * w * h * sizeof(float));
	FORJ(h) FORI(w) {
		float g = x[j*w+i];
		xs[j][i] = x[j][i];
...
void gblur_symmetric(float *y, float *x, int w, int h, int pd, float s)
{
	...
	FORL(pd) {
		FORI(w*h)
			c[i] = x[i*pd + l];
		if (s)
			gblur_symmetric_gray(gc, c, w, h, s);
