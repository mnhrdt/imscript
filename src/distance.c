...
#include "smapa.h"
SMART_PARAMETER_SILENT(DISTANCE_SLOW,0)

int main(int c, char *v[])
{
	if (c != 3 && c != 4 && c != 5) {
		fprintf(stderr, "usage:\n\t%s width height [points [outimage]]\n", *v);
		//                          0 1     2       3       4
		return 1;
	}
	int w = atoi(v[1]);
	int h = atoi(v[2]);
	char *filename_points = c > 3 ? v[3] : "-";
	char *filename_outimage = c > 4 ? v[4] : "-";

	int n;
	FILE *f = xfopen(filename_points, "r");
	float *p = read_ascii_floats(f,  &n);
	xfclose(f);
	if (2*(n/2) != n)
		fail("i'm expecting a list of points");
	n /= 2;
