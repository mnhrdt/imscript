...
static float pamle_iteration(float *x, int w, int h, int (*mask)[2], int nmask,
		float P)
{
	float actus = 0;
	float actumax = 0;
...
// fill the holes of the image x using a p harmonic function
static void p_harmonic_extension_with_init(
		...
		)
{
	...
	{
		float u = pamle_iteration(y, w, h, mask, nmask, p);
...
SMART_PARAMETER(AMLE_ONLY,0)

void pamle_recursive(float *out, float *in, int w, int h,
		float p, int niter, int scale)
{
	...
	if (scale > 1)
	{
		...
		pamle_recursive(outs, ins, ws, hs, p, niter, scale - 1);
	...
	p_harmonic_extension_with_init(out, in, w, h, p, niter, init);
	free(init);
}
...
int main(int argc, char *argv[])
{
	if (argc != 7) {
		...
		"%s P NITER NS data.png mask.png out.png\n", *argv);
		//0 1 2     3  4        5        6
		return 1;
	}
	float p = atof(argv[1]);
	...
	pamle_recursive(out, in, *w, *h, p, niter, nscales);
