...
//static void wait_until_next_expose(struct _FTR *f)
//{
//	XEvent event;
//	while (1) {
//		XNextEvent(f->display, &event);
//		fprintf(stderr, "w_u_n_e event: %d\t\"%s\"\n", event.type,
//				event_names[event.type]);
//		if (event.type != Expose) continue;
//
//		// at the first Expose event, create the ximage
//		if (!f->ximage)
...
//struct FTR ftr_new_window_with_image_uint8_rgba(unsigned char *x, int w, int h)
//{
//	struct _FTR f[1];
//
//	f->display = XOpenDisplay(NULL);
